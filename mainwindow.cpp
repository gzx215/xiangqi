            ...
            for(int i=0;i<point.size();++i)
            {
                ...
                if(p==point[i])
                {
                    ...
                    if(s.length()==0)
                    {
                        ...
                        q+=QString::number(q.length());
                        tcpsocket->write(q.toLatin1());
                    }
                    else if(s.length()==1)
                    {
                        QString q="2"+QString::number(posid)+QString::number(my_chess[posid].show);
                        q+=QString::number(my_chess[posid].pos.x());
                        q+=QString::number(my_chess[posid].pos.y());
                        q+=QString::number(isorder);
                        data=q+s;

                        data+=QString::number(q.length());
                        tcpsocket->write(data.toLatin1());
                    }
                    else if(s.length()==2)
                    {
                        QString q="3"+QString::number(posid)+QString::number(my_chess[posid].show);
                        q+=QString::number(my_chess[posid].pos.x());
                        q+=QString::number(my_chess[posid].pos.y());
                        q+=QString::number(isorder);
                        data=q+s;
...
void MainWindow::ReadInformation()
{
    ...
    else if(temp[0]=='2')
    {
        ...
        QString iii=*(temp.end()-2);
        int numm=iii.toInt();
        my_chess[numm].show=false;
        my_chess[numm].pos=QPoint(9,9);
    }
    else if(temp[0]=='3')
    {
        QString i,j,k,m,p,n;
        ...
        QString iii=*(temp.end()-3);
        QString iiii=*(temp.end()-2);
        int numm=(iii+iiii).toInt();
        my_chess[numm].show=false;
        my_chess[numm].pos=QPoint(9,9);
    }
